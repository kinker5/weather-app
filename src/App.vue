...
export default {
	...
	data: () => ({
		...
		api_key: "e1a64de13e0c5c5f2c7970480798d9f3",
		query: "Brookfield, WI, USA",
		weather: {},
		lat: '43.061615',
		long: '-88.107948'
	}),
	methods: {
		fetchWeather() {
			axios
				...
				.catch((error) => {
					console.log(error)
					axios
					.get(`${this.api_url}weather?lat=${this.lat}&lon=${this.long}&units=metric&appid=${this.api_key}`)
					.then((response) => (this.weather = response.data))
				});
		},
		getAddressData (addressData) {
			...
			this.query = addressData.locality + ',' + addressData.administrative_area_level_1 + ',' + addressData.country
			this.fetchWeather()
        }
	},
	computed: {
